...
static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Wildlifecoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Wildlifecoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
        ...
        if (fTestNet) {
                if (pindexLast->nHeight+1 >= 2116) { DiffMode = 2; } // wildlifecoin, 100 blocks after first legacy diff adjustment
        }
        else {         
        	if (pindexLast->nHeight+1 >= 26754) { DiffMode = 2; }  //wildlifecoin, 5 days after 27/01/2014 12:00 UTC
        }
